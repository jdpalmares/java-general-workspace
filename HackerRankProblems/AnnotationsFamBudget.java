...
// Here, we define an annotation , where  is the only member in that custom annotation. 
// The  takes only  type values, and the default is "GUEST". 
...
// Here, we partially define an annotation,  and a class, .
// In this problem, we give the user role and the amount of money that a user spends as inputs. 
// Based on the user role, you have to call the appropriate method in the  class. 
// If the amount of money spent is over the budget limit for that user role, 
// it prints Budget Limit Over.

// Your task is to complete the  annotation and the  class so that the  class works perfectly 
// with the defined constraints.

// Note: You must complete the  incomplete lines in the editor. 
...
// Input Format
// The first line of input contains an integer  representing the total number of test cases. 
// Each test case contains a string and an integer separated by a space on a 
// single line in the following format: UserRole MoneySpend

// Constraints Name contains only lowercase English letters.
...
public class AnnotationsFamBudget {
